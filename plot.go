...
func Plot(metrics model.Matrix, title, format string, subsequence int) (io.WriterTo, error) {
	p, err := plot.New()
	if err != nil {
	...
	for s, sample := range metrics {
		lineData := make(plotter.XYs, len(sample.Values))
		sig := make([]float64, len(sample.Values))
		for i, v := range sample.Values {
			lineData[i].X = float64(v.Timestamp.Unix())
		...
		if err != nil {
			return nil, fmt.Errorf("failed to init matrixprofile: %v", err)
		}

		if err = mp.Stomp(runtime.NumCPU()); err != nil {
			return nil, fmt.Errorf("failed to calc matrixprofile: %v", err)
		}

		discords, err := mp.TopKDiscords(8, mp.M/2)
		if err != nil {
			return nil, fmt.Errorf("failed to calc discords: %v", err)
		}

		scatterData := make(plotter.XYs, len(discords))
		for k, v := range discords {
			scatterData[k].X = lineData[v].X
			scatterData[k].Y = lineData[v].Y
		}
